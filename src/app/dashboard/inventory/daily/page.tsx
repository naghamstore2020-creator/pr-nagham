...
export default function DailyInventoryPage() {
  ...
  const handleStartProcessing = async () => {
    ...
    try {
      ...
      if (res.success && res.exportFileUrl) {
        setResults(res);
        toast.success("تم إتمام معالجة الجرد اليومي وتحديث الكميات بنجاح!");
      } else {
        toast.error(res.error || "فشل معالجة الجرد اليومي");
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">معالجة الجرد اليومي</h1>
        <p className="text-zinc-400 text-xs mt-1">
          يقوم هذا الخيار بمطابقة رمز SKU وتنزيل الكميات الناقصة في المتجر، مع منع زيادة الكميات وتصفير المنتجات المفقودة.
        </p>
      </div>
        ...
        <Card className="border-amber-500/20 bg-amber-500/5 backdrop-blur-md">
          <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-zinc-400 text-xs mt-1">
                يرجى رفع منتجات المتجر أو ملف الكميات من المتجر وملف النظام (جرد أساسي) أولاً لتتمكن من تشغيل عملية الجرد.
              </p>
            </div>
            ...
            <div className="space-y-6 animate-slide-up">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card className="border-zinc-800 bg-zinc-900/50">
                  <CardContent className="p-4 flex items-center justify-between">
                    <div>
                      <p className="text-zinc-500 text-[10px]">زيادات تم حجبها (حماية)</p>
                      <h4 className="text-lg font-bold text-white mt-1">{results.stats.totalBlocked}</h4>
                    </div>
              ...
              {results.details && results.details.length > 0 && (
                <InventoryPreview details={results.details} mode="daily" />
              )}
              ...
              <Card className="border-emerald-500/20 bg-emerald-500/5">
                <CardContent className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-bold text-white">الملف جاهز للتنزيل!</p>
                      <span className="text-xs text-zinc-400">
                        تم الحفاظ على الترتيب الأصلي للصفوف والتنسيقات.
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex gap-2.5 p-3 rounded-lg bg-zinc-900/20 border border-zinc-800 text-xs text-zinc-400">
                ...
                <p className="leading-relaxed">
                  <strong>ملاحظة حول قواعد الجرد اليومي:</strong> يتم مطابقة رمز الـ SKU لكل منتج. إذا كانت كمية النظام أقل من كمية المتجر، يتم تعديلها للكمية الأقل. أما إذا كانت كمية النظام أعلى، يتم الاحتفاظ بالكمية القديمة وتصنيف العملية كـ "زيادة محجوبة" لمنع الأخطاء غير المقصودة. المنتجات التي لا تمتلك رمز SKU أو غير متوفرة في النظام تصبح كميتها صفر.
                </p>
              </div>
            </div>
          )}
